...
{
    return parser.isSet("headless") || parser.isSet("exit-after-load") || parser.isSet("automation");
}
...
int main(int argc, char *argv[])
{
    ...
    if (!fileToOpen.isEmpty()) {
        ...
        window.applyCommandLineOptions(parser);
        if (parser.isSet("exit-after-load")) {
            return 0;
        }
    }
    ...
    // If headless without file or exitAfterLoad, just exit
    if (headless) {
        return 0;
    }
