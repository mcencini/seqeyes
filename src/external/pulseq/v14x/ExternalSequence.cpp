...
bool ExternalSequence::load(std::istream& data_stream, load_mode loadMode /*=lm_singlefile*/)
{
	...
	// Check for v.1.4.x compatibility (this loader only supports v.1.4.x)
	if (version_major != 1 || version_minor != 4)
	{
		print_msg(ERROR_MSG, std::ostringstream().flush() << "*** ERROR: This loader (v1.4.2) only supports Pulseq file version 1.4.x. Loaded file version: " << version_major << "." << version_minor << "." << version_revision);
		return false;
	}
...
bool ExternalSequence::decodeBlock(SeqBlock *block)
{
	...
	if (block->isRF())
	{
		...
		if (!decompressShape(shapePhase,&waveform_p[0]))
			return false;
		// Scale phase by 2pi
		std::transform(waveform_p.begin(), waveform_p.end(), waveform_p.begin(), std::bind(std::multiplies<float>(),TWO_PI, _1));
