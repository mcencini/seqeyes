...
#include "../../compat_bind.hpp"  // Compatibility for std::bind1st in C++17
extern "C" {
    #include "md5.h"
}
...
bool ExternalSequence::decodeBlock(SeqBlock *block)
{
	...
	if (block->isRF())
	{
		...
		std::transform(
			...
			waveform_p.end(),
			waveform_p.begin(),
			[](float x) { return x * static_cast<float>(TWO_PI); }
		);
...
int ExternalSequence::decodeLabel(ExtType exttype, int& nVal, char* szLabelID, LabelEvent& label)
{
	...
	if (it==m_labelMap.mapStrToLabel.end())
	{
		...
		label.flagVal = std::make_pair(FLAG_UNKNOWN, false);
		
		print_msg(WARNING_MSG, std::ostringstream().flush() << "*** WARNING: unknown label specification: " << szLabelID << " (assigned ID: " << customLabelId << ", value: " << nVal << ")\n");
		return 1; // Still return 1 to indicate warning, but label is now properly set
	}
...
void LabelStateAndBookkeeping::updateLabelValues(SeqBlock* pBlock)
{
    ...
    for (id = 0; id < Tmp_labelset.size(); ++id)
    {
        ...
        if (Tmp_labelset[id].numVal.first != LABEL_UNKNOWN && 
            Tmp_labelset[id].numVal.first >= 0 && 
            Tmp_labelset[id].numVal.first < NUM_LABELS)
        {
        ...
        if (Tmp_labelset[id].flagVal.first != FLAG_UNKNOWN && 
            Tmp_labelset[id].flagVal.first >= 0 && 
            Tmp_labelset[id].flagVal.first < NUM_FLAGS)
        {
...
std::string ExternalSequence::GetUnknownLabelName(int labelId) const
{
    // Use the existing label map system to get the label name
    auto it = m_labelMap.mapLabelIdToStr.find(labelId);
    if (it != m_labelMap.mapLabelIdToStr.end()) {
        return it->second;
    }
